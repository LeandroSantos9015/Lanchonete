...
namespace Lanchonete.Areas.Admin.Controllers
{
    ...
    public class AdminImagensController : Controller
    {
        ...
        {
            _config = configurationImagens.Value;
        ...
        public async Task<IActionResult> UploadFiles(List<IFormFile> files)
        {
            ...
            if (files.Count() == 10)
            {
                ViewData["Erro"] = "Error: Quantidade excede o limite";
                return View(ViewData);
            }
        ...
        {
            string _imagemData = Path.Combine(_hostingEnvironment.WebRootPath, _config.NomePastaImagensProdutos + "\\" + fname);

            if (System.IO.File.Exists(_imagemData))
            {
                System.IO.File.Delete(_imagemData);
                ViewData["Deletado"] = $"Arquivo(s) {_imagemData} deletado com sucesso";
            }
            return View("index");  
        }
    }
